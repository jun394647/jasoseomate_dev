...
export async function generateCompanyReport(userId: string, companyId: string): Promise<{
  report: string;
  costUsd: number | null;
}> {
  ...
  const company = (await db
    ...
    .get(companyId, userId)) as Company | undefined;
...
위 자료를 종합해 기업 분석 리포트를 작성해줘. 지정된 출력 형식만 사용해.`;
  ...
  const ragText = [company.analysis, company.talent_profile, ...newsList, report]
    .filter(Boolean)
    .join("\n\n");
  await reindexSource(userId, "company", companyId, ragText);
