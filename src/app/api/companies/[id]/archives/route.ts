...
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/companies/[id]/archives">) {
  ...
  const rows = (await db
    .prepare(
      `SELECT ca.* FROM company_archives ca
       JOIN companies c ON c.id = ca.company_id
       WHERE ca.company_id = ? AND c.user_id = ? ORDER BY ca.created_at DESC`
    )
    .all(id, session.id)) as CompanyArchive[];
...
export async function POST(req: NextRequest, ctx: RouteContext<"/api/companies/[id]/archives">) {
  ...
  await reindexSource(session.id, "company_archive", id, `${title.trim()}\n\n${content.trim()}`);
